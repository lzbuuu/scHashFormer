...
## Architecture
The neural network architecture of scHashFormer. Tokenization module aims to generate the token sequence, which contains a multi-head, learnable LSH encoder that produces continuous hash embeddings and binary hash codes; identical codes define hash neighbourhoods. A contrastive objective tightens within-neighbourhood similarity and separates cross-neighbourhood pairs, and a decoder reconstructs gene expression to retain biological information. Embedding module aims to learn cell embedding: for each cell, tokenised same-neighbourhood cells are encoded by a Transformer to obtain structure-aware embeddings; a self-supervised clustering enforces structural consistency within the cell embedding space, and a self-supervised reconstruction preserves the biological integrity. The workflow preserves local manifold structure while avoiding the construction of all pairwise relationships.![fram1 (1)](./scHashFormer.png)
